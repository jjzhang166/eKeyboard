...
//创建键盘方法（必需先创建）
function eKeyboard(Usercolor,KeyboardDiv){
	color = Usercolor;
	keyboardDiv = KeyboardDiv;
	document.querySelector(KeyboardDiv).innerHTML+="<div id='keyboard'></div>";
...
function addfont(){
	...
	for(var i = 0;i<zimu3.length;i++){
		if(isLock == 1||isShift==1){
			...
			eval("document.querySelector('#button-"+zimu3[i]+"').innerHTML = document.querySelector('#button-"+zimu3[i]+"').innerHTML.toLowerCase()");
...
//删除keyboard(会清楚Keyboard所在的div里的全部)
function deleteKeyboard(){
	clearInterval(Keyboardtimer);
	document.querySelector(keyboardDiv).innerHTML = "";
}
